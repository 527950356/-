    ...
    <div class="order-item" v-for="item of list" :key="item.id">
        ...
        <div class="item-bottom">
            ...
            <el-button round class="change" v-show="item.orderActive == '0'" @click="change(item)">
                <span>取消订单</span></el-button>

                 <el-button round class="change1" v-show="(item.orderActive == '1'||item.orderActive == '4'||item.orderActive == '5')&&orderTye==1" @click="delect(item)"><span>删除订单</span></el-button>
                        <el-button round class="change" v-show="item.orderActive == '2'" @click="change2(item)"><span>确认收货</span></el-button>


            <el-button round class="change" @click.stop="evaluate(item)" v-show="item.orderActive == '4'" >
                <span>评价</span>
        </el-button>
    </div>
    <!-- 第二 -->
        <div class="order-item" v-for="item of list3" :key="item.id" v-show="orderTye=='4'">
        <div class="item-top">
            <img src="../../assets/我的订单.png" alt="">
            <span>{{item.orderCode}}</span>
            <span>{{item.orderActive ==='0'?'已下单':
